...
function Base.push!(sys::System, a::Atom, cor, vel, ad)
    n = length(sys.atoms)
    asys = deepcopy(sys)
    push!(asys.atoms, Atom(index=n+1, atom_type=a.atom_type, mass=a.mass, charge=a.charge, σ=a.σ, ϵ=a.ϵ))
    push!(asys.coords, cor)
    push!(asys.velocities, vel)
    push!(asys.atoms_data, ad)
    push!(asys.masses, a.mass)
    asys.total_mass = sum(asys.masses)
    if all(length(asys.atoms) == length(l) for l in (asys.atoms, asys.coords, asys.velocities, asys.atoms_data, asys.masses))
        println("Only added atom, but no interactions added")
        return asys
    else
        println("Failed to add atom to system")
        return sys
    end
end

function Base.append!(sys::System, l_a, l_cor, l_vel, l_ad)
    n = length(sys.atoms) + 1
    asys = deepcopy(sys)
    for (a, cor, vel, ad) in zip(l_a, l_cor, l_vel, l_ad)
        push!(asys.atoms, Atom(index=n, atom_type=a.atom_type, mass=a.mass, charge=a.charge, σ=a.σ, ϵ=a.ϵ))
        push!(asys.coords, cor)
        push!(asys.velocities, vel)
        push!(asys.atoms_data, ad)
        push!(asys.masses, a.mass)
    ...
    asys.total_mass = sum(asys.masses)
    if all(length(asys.atoms) == length(l) for l in (asys.atoms, asys.coords, asys.velocities, asys.atoms_data, asys.masses))
        println("Only added atoms, but no interactions added")
        return asys
    else
        println("Failed to add atoms to system")
        return sys
    end
end

Base.append!(sys::System, sys_app::System) = append!(sys, sys_app.atoms, sys_app.coords, sys_app.velocities, sys_app.atoms_data)
