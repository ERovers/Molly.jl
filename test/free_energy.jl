@testset "OpenFE TYK2 comparison" begin
    ...
    ff_A = MolecularForceField(FT, joinpath.(ff_dir, ["tip3p_standard.xml", "amber14/protein.ff14SB.xml"])...,
                                            joinpath(data_dir, "ejm31_new.xml"),
                                            ; units=true)

    ff_B = MolecularForceField(FT, joinpath.(ff_dir, ["tip3p_standard.xml", "amber14/protein.ff14SB.xml"])...,
                                            joinpath(data_dir, "ejm50_new.xml")
                                            ; units=true)

    # --- Load Systems ---
    sysA = System(
            joinpath(data_dir,"system_A.pdb"),
            ff_A;
    ...
    sysB = System(
            joinpath(data_dir, "system_B.pdb"),
            ff_B;
            nonbonded_method=:pme,
            center_coords=false,
            dist_cutoff=FT(0.9)u"nm",
    ...
    # --- Hybrid System Setup ---
    sys, mappingA, mappingB = Hybrid_system(FT, , sysA, sysB, FT(0.0), mapping, core_mapAB, "";
                                    scheduler=Molly.OpenMMTestScheduler())

    #--- Load Positions ---
    openmm_coords_fp = "positions_openmm.txt"
    coords_openmm = Dict()
    chains = "ABCDE"
    res_id = 0
    resi_n = 0
    ...
    for inter in inters
        ...
        sys_part = System(
            atoms=test_sys.atoms,
            coords=test_sys.coords,
            boundary=test_sys.boundary,
            pairwise_inters=pin,
            specific_inter_lists=sils,
            general_inters=gis,
            neighbor_finder=test_sys.neighbor_finder,
        )
        ...
        etol = (inter == "PME" ? 1e-3 : 1e-5)u"kJ * mol^-1"
        @test abs(E_molly - E_openmm) < etol
    end


    # --- Energy and Forces for λ=1 ---
    test_sys = inject_lambda(test_sys, FT(1.0), AT)
    inters = (
        "bond_only", "angle_only", "torsion_only", "Nonbonded", "PME", "all"
    )
    for inter in inters
